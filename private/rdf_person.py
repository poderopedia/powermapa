...
familiares=db((db.relFamiliar.is_active==True)&(db.relFamiliar.origenP==db.persona.id)&(db.persona.is_active==True)
            & (db.relFamiliar.parentesco==db.tipoParentesco.id)).select()
...
relations = db((db.relPersona.is_active==True)&(db.relPersona.origenP==db.persona.id)&(db.persona.is_active==True)).select()
...
orgsrelations = db((db.RelPersOrg.is_active==True)&(db.persona.id==db.RelPersOrg.origenP)&(db.persona.is_active==True)
                   &(db.Organizacion.id==db.RelPersOrg.destinoO)&(db.Organizacion.is_active==True)
                   &(db.RelPersOrg.specificRelation==db.tipoRelacionP20.id)).select()
for orgs in orgsrelations:
    person=TEST[str(orgs.RelPersOrg.origenP)]
...
##orgsrelations = db((db.relPersOrg.is_active==True)&(db.persona.id==db.relPersOrg.origenP)&(db.persona.is_active==True)
##                &(db.Organizacion.id==db.relPersOrg.destinoO)&(db.Organizacion.is_active==True)).select()
##for orgs in orgsrelations:

# Print the serialized graph
data=jg.serialize(format='xml')
filename = open('applications/PowerMapa/private/person.rdf','w')
filename.write(data)
filename.close()
